    ...
    <main class="flex-1 max-w-5xl mx-auto w-full pt-8 px-4 flex flex-col">
      <div class="flex items-center justify-between mb-6">
        <h1 class="text-2xl font-bold flex items-center gap-2">
          <el-icon class="text-blue-500"><Document /></el-icon> 我的帖子
        </h1>
      ...
      <el-card shadow="never" class="border-none rounded-2xl flex-1 flex flex-col min-h-[60vh]" v-loading="loading">
        ...
        <el-empty v-else-if="!loading" description="你还没有发布过帖子" class="my-auto" />
...
const fetchPosts = async () => {
  ...
  try {
    const res: any = await request.get('/posts/', {
      params: { author_id: authStore.user.id, page: page.value, page_size: pageSize.value }
    })
    // 降序排序
    posts.value = (res.items || []).sort((a: any, b: any) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime())
